...
export default async function SuppliersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) { const query = parseListQuery(await searchParams); const result = await listParties("suppliers", query); return <AppShell active="erp-suppliers" eyebrow="Cadastros" title="Fornecedores"><PartyTable {...result} basePath="/erp/fornecedores" query={query.q} kind="fornecedor" /></AppShell>; }
