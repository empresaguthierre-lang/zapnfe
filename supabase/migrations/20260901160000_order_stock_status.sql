...
do $$ begin
  create type public.order_stock_status as enum ('unreserved', 'reserved', 'partial', 'released', 'fulfilled');
exception
  when duplicate_object then null;
end $$;

alter table public.orders
add column if not exists stock_status public.order_stock_status not null default 'unreserved';

-- Re-create save_order_review to support force_approval and stock_status
drop function if exists public.save_order_review(uuid, uuid, timestamptz, jsonb, numeric, numeric, text, boolean);
create or replace function public.save_order_review(
  ...
  force_approval boolean default false
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_order_stock_status public.order_stock_status := 'unreserved';
begin
  ...
  for item in select value from jsonb_array_elements(review_items) loop
    ...
    if mark_approved and resolved_product.track_stock then
      ...
      if v_available < item_quantity then
        if not force_approval then
          raise exception 'INSUFFICIENT_STOCK:%', resolved_product.name;
        end if;
        v_is_partial := true;
        v_reserve_qty := greatest(0::numeric, v_available);
      else
        v_reserve_qty := item_quantity;
      end if;
      ...
      if v_reserve_qty > 0 then
        ...
        insert into public.stock_reservations (
          ...
          expected_organization_id, v_warehouse_id, resolved_product.id, 'order_item', target_item.id, v_reserve_qty, 'active'
        );
      end if;
    end if;
  ...
  if mark_approved then
    if v_is_partial then
      v_order_stock_status := 'partial';
    else
      v_order_stock_status := 'reserved';
    end if;
  else
    v_order_stock_status := target_order.stock_status;
  end if;
  ...
  set discount = round(review_discount, 2),
      ...
      stock_status = v_order_stock_status,
      reviewed_at = now(),
  ...
  if mark_approved then
    insert into public.business_events (
      ...
      target_order.id,
      current_user_id,
      jsonb_build_object('order_number', target_order.number, 'stock_status', v_order_stock_status)
    );
  end if;

  return jsonb_build_object('status', resulting_status, 'updated_at', saved_updated_at, 'stock_status', v_order_stock_status);
end;
$$;
revoke all on function public.save_order_review(uuid, uuid, timestamptz, jsonb, numeric, numeric, text, boolean, boolean) from public, anon;
grant execute on function public.save_order_review(uuid, uuid, timestamptz, jsonb, numeric, numeric, text, boolean, boolean) to authenticated, service_role;
