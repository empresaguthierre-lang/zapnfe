-- Reapplies the hardened review RPC under a new migration so existing
-- environments receive the concurrency changes even if 20260831133100 was
-- already applied. It also binds writes to the application-selected company.
...
begin
  if not exists (
    ...
    where conrelid = 'public.orders'::regclass
      and conname = 'orders_reviewed_by_fkey'
  ) then
    alter table public.orders
  ...
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'public.orders'::regclass
      and conname = 'orders_approved_by_fkey'
  ) then
    alter table public.orders
...
drop function if exists public.save_order_review(uuid, jsonb, numeric, numeric, text, boolean);
drop function if exists public.save_order_review(uuid, timestamptz, jsonb, numeric, numeric, text, boolean);
...
revoke all on function public.save_order_review(uuid, uuid, timestamptz, jsonb, numeric, numeric, text, boolean) from public, anon;
grant execute on function public.save_order_review(uuid, uuid, timestamptz, jsonb, numeric, numeric, text, boolean) to authenticated, service_role;

comment on function public.save_order_review is 'Atomically persists human order review for the application-selected organization with optimistic concurrency. Approved or closed orders are immutable through this workflow.';
