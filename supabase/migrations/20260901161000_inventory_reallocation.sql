...
create table if not exists public.stock_reservation_audits (
  ...
  source_reservation_id uuid not null references public.stock_reservations(id) on delete cascade,
  ...
  target_reservation_id uuid references public.stock_reservations(id) on delete cascade,
  target_order_id uuid references public.orders(id) on delete set null,
  target_order_item_id uuid references public.order_items(id) on delete set null,
  target_customer_id uuid references public.customers(id) on delete set null,

  quantity numeric not null,

  source_quantity_before numeric not null,
  source_quantity_after numeric not null,
  target_quantity_before numeric not null,
  target_quantity_after numeric not null,
...
create policy audits_select on public.stock_reservation_audits for select to authenticated using (public.is_organization_member(organization_id));
...
begin
  ...
  -- Check if target actually needs stock
  ...
  -- Update or create target reservation
  if found then
  ...
  -- Recalculate source order stock_status
  -- If all items in source order are fully covered, it's reserved, else partial, unless 0 total reservations then unreserved
  -- But to keep it simple and accurate, we can just check if sum(res.quantity) = sum(item.quantity)
  -- We'll do a quick subquery for source order
  ...
  -- Recalculate target order stock_status
