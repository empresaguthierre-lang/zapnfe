...
begin
  ...
  if not found then
    ...
    if v_org_profile.tax_regime is null then
      v_issues := v_issues || jsonb_build_object('code', 'ISSUER_TAX_REGIME_MISSING', 'severity', 'error', 'entity', 'organization', 'message', 'Regime tributǭrio do emitente nǜo informado.', 'action', jsonb_build_object('label', 'Corrigir', 'href', '/fiscal/configuracoes'));
      v_errors := v_errors + 1;
    end if;
    if v_org_profile.fiscal_address is null then
      v_issues := v_issues || jsonb_build_object('code', 'ISSUER_FISCAL_ADDRESS_MISSING', 'severity', 'error', 'entity', 'organization', 'message', 'Endereo fiscal do emitente nǜo informado.', 'action', jsonb_build_object('label', 'Corrigir', 'href', '/fiscal/configuracoes'));
      v_errors := v_errors + 1;
    end if;
  end if;
  ...
  if not (v_restrictions->>'allowed')::boolean then
    v_issues := v_issues || jsonb_build_object(
      ...
      'action', jsonb_build_object('label', 'Ver restries', 'href', '/clientes/' || v_order.customer_id || '?tab=restricoes')
    );
    v_errors := v_errors + 1;
  end if;
  ...
  if not found then
    ...
    if v_customer_profile.fiscal_address_id is null then
      v_issues := v_issues || jsonb_build_object('code', 'CUSTOMER_FISCAL_ADDRESS_MISSING', 'severity', 'error', 'entity', 'customer', 'entity_id', v_order.customer_id, 'message', 'Endereo fiscal do cliente nǜo informado.', 'action', jsonb_build_object('label', 'Corrigir', 'href', '/clientes/' || v_order.customer_id || '?tab=fiscal'));
      v_errors := v_errors + 1;
    end if;
  end if;
